      ...
      <div class="card">
        <div class="card-body">
          <form action="" method="POST">
            ...
            <input type="hidden" name="id" value="<?= $workshopParicipant->id ?>">
            <div class="row">
              <div class="row">
                <div class="col-md-12">
                  <div class="mb-3">
                    <label for="namaKategoriPeserta" class="form-label">Kategori Peserta</label>
                    <select class="form-select select2 <?= form_error('pc_id') ? 'is-invalid' : ''; ?>" name="pc_id" id="namaKategoriPeserta">
                      <?php foreach ($participantCategories as $row) : ?>
                        <option value="<?= $row->id ?>" <?= ($row->id == $workshopParicipant->pc_id) ? 'selected' : ''; ?>>
                          <?= $row->name ?>
                        </option>
                      <?php endforeach ?>
                    </select>
                  ...
                  <div class="mb-3">
                    ...
                    <select class="form-select select2 <?= form_error('pt_id') ? 'is-invalid' : ''; ?>" name="pt_id">
                      ...
                      <?php foreach ($priceTypes as $row) : ?>
                        <option value="<?= $row->id ?>" <?= ($row->id == $workshopParicipant->pt_id) ? 'selected' : ''; ?>>
                          <?= $row->name ?>
                        </option>
                      <?php endforeach ?>
                    </select>
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    ...
                    <input type="text" name="price" id="price" class="form-control <?= form_error('price') ? 'is-invalid' : ''; ?>" onkeyup="formatRupiah(this)" value="<?= set_value('price') ? set_value('price') : $workshopParicipant->price; ?>">
                    <div class="invalid-feedback">
                      <?= form_error('price'); ?>
                    </div>
                  </div>
